...
import { Loader2, ArrowLeft, Armchair, Ticket, Coffee, MessageSquare } from 'lucide-react';
...
const OrbitWaitingPage = () => {
    ...
    const fetchRoom = async () => {
      try {
        ...
        if (active) {
          setRoom(data);
          setLoading(false);
          // If room is already closed, redirect to boarding
  ...
  // Sync room updates in real-time
  useRealtimeTopic(REALTIME_TOPICS.orbitRoom(roomUuid), (updatedRoom) => {
    ...
    setChanging(true);
    try {
      // Clear combos and completed flag in backend
  ...
  const myMemberInfo = room?.members?.find((m) => m.userUuid === currentUserUuid);
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] pt-24 pb-12 flex flex-col items-center px-4 md:px-8 relative z-10">
      ...
      <div className="w-full max-w-5xl bg-[#111827]/80 backdrop-blur-xl rounded-3xl shadow-2xl overflow-hidden border border-white/10 ring-1 ring-white/5 flex flex-col md:flex-row">
        {/* LEFT COLUMN: Status & Selection Summary */}
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mb-8">
              ...
              <div>
                ...
                <p className="text-[10px] text-zinc-400 font-semibold mt-0.5">ID: {roomUuid}</p>
              </div>
            </div>

            {/* Waiting message banner */}
            <div className="bg-amber-500/10 border border-amber-500/20 rounded-2xl p-5 mb-8 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-zinc-300 mt-1 leading-relaxed">
                  Bạn đã hoàn tất chọn ghế và nước. Đơn hàng của bạn sẽ được Host trả toàn bộ. Vui lòng giữ trang này mở để tự động nhận vé sau khi Host thanh toán xong.
                </p>
              </div>
            </div>
            ...
            <div className="space-y-6">
              <h3 className="text-xs font-black uppercase tracking-wider text-zinc-400 border-l-2 border-red-500 pl-2">Lựa chọn của bạn</h3>
              ...
              <div className="flex items-center gap-4 bg-white/5 p-4 rounded-xl border border-white/5">
                ...
                <div>
                  ...
                  <span className="text-[10px] text-zinc-400 font-semibold block mt-0.5">Số lượng: {mySeats.length}</span>
                  {mySeats.length > 0 && (
                    ...
                    <div className="mt-1 space-y-1">
                      {myCombos.map((combo, idx) => (
                        <span key={idx} className="text-xs font-black text-yellow-400 block">
                          {combo.quantity}x Combo ({combo.comboUuid?.substring(0, 8)})
                        </span>
                      ))}
                    </div>
                  )}
