 ...
 */
const mapBackendRoles = (roles: string[]): User['roles'] => {
...
class AuthService {
  ...
  async login(credentials: LoginCredentials): Promise<AuthResponse> {
    try {
      ...
      const response = await this.api.post<{ data: JwtResponse }>('/api/auth/login', {
        email: credentials.email,
        password: credentials.password,
      });

      // BE trả về ApiResponse: { code, message, data: { accessToken, tokenType, email, roles } }
      const jwtData: JwtResponse = response.data.data ?? (response.data as unknown as JwtResponse);
  ...
  async register(_: RegisterCredentials): Promise<AuthResponse> {
    throw new Error('Registration is not supported by the current backend version.');
  }

  async loginWithGoogle(_: string): Promise<AuthResponse> {
    throw new Error('Social login is not supported by the current backend version.');
  }

  async loginWithApple(_: string): Promise<AuthResponse> {
    throw new Error('Social login is not supported by the current backend version.');
  }

  async forgotPassword(_: ForgotPasswordRequest): Promise<{ message: string }> {
    throw new Error('Password recovery is not supported by the current backend version.');
  }

  async resetPassword(_: ResetPasswordRequest): Promise<{ message: string }> {
  ...
  async refreshToken(): Promise<string> {
    try {
      ...
      const response = await axios.post<{ data: JwtResponse }>(
        `${API_BASE_URL}/api/auth/refresh`,
        { refreshToken: currentRefreshToken }
      );

      const jwtData = response.data.data ?? (response.data as unknown as JwtResponse);
